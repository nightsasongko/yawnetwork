...
class Dasar_model extends CI_Model 
{
	...
	// fungsi mencari random string unik yg dibandingkan dengan isi di tabelnya, untuk memastikan ga ada yang dobel
	function getRandomKey($namatabel,$namafield, $jml_char)
	{
		$string = '';
		...
		$sql = "select count($namafield) as jml from $namatabel where $namafield='$string'";	
		$res = $this->db->query($sql);
		$row = $res->row_array();		
		$jml = $row['jml'];
		if($jml > 0)
		{
			return $this->getRandomKey($namatabel,$namafield, $jml_char);
		}
		else
		{
			return $string;
